...
%token	<n>		PLUS MINUS MULT DIV DUP
...
%type	<sym>	decl8_list decl16_list dup_expr
%type <sym> crit_expr crit8_expr crit16_expr crit8_prim crit16_prim
...
pseudo_op:
		...
		| DECL_BYTE expr8 dup_expr 	{ code->putSyms($2, $3, IM8); }
		| DECL_WORD decl16_list			{ code->putSym($2, IM16); }
		| DECL_WORD expr16 dup_expr { code->putSyms($2, $3, IM16); }
...
dup_expr:	DUP '(' crit_expr ')'	{ $$ = $3; }
			;

/* a critical expression must be resolvable during the first pass */
crit_expr: crit8_expr | crit16_expr
			;

crit8_expr: crit8_prim
			| crit8_expr PLUS crit8_expr		{ $$ = table->opeval(PLUS, IM16, table->mklist($1, $3)); }
			| crit8_expr MINUS crit8_expr		{ $$ = table->opeval(MINUS, IM16, table->mklist($1, $3)); }
			;
...
crit16_expr: crit16_prim
			| crit8_expr PLUS crit16_expr		{ $$ = table->opeval(PLUS, IM16, table->mklist($1, $3)); }
			| crit16_expr PLUS crit8_expr		{ $$ = table->opeval(PLUS, IM16, table->mklist($1, $3)); }
			| crit16_expr PLUS crit16_expr	{ $$ = table->opeval(PLUS, IM16, table->mklist($1, $3)); }
			| crit8_expr MINUS crit16_expr	{ $$ = table->opeval(MINUS, IM16, table->mklist($1, $3)); }
			| crit16_expr MINUS crit8_expr	{ $$ = table->opeval(MINUS, IM16, table->mklist($1, $3)); }
			| crit16_expr MINUS crit16_expr	{ $$ = table->opeval(MINUS, IM16, table->mklist($1, $3)); }
			| crit8_expr MULT crit8_expr		{ $$ = table->opeval(MULT, IM16, table->mklist($1, $3)); }
			| crit8_expr MULT crit16_expr		{ $$ = table->opeval(MULT, IM16, table->mklist($1, $3)); }
			| crit16_expr MULT crit8_expr		{ $$ = table->opeval(MULT, IM16, table->mklist($1, $3)); }
			| crit16_expr MULT crit16_expr	{ $$ = table->opeval(MULT, IM16, table->mklist($1, $3)); }
			| crit8_expr DIV crit8_expr			{ $$ = table->opeval(DIV, IM16, table->mklist($1, $3)); }
			| crit8_expr DIV crit16_expr		{ $$ = table->opeval(DIV, IM16, table->mklist($1, $3)); }
			| crit16_expr DIV crit8_expr		{ $$ = table->opeval(DIV, IM16, table->mklist($1, $3)); }
			| crit16_expr DIV crit16_expr		{ $$ = table->opeval(DIV, IM16, table->mklist($1, $3)); }
			;
